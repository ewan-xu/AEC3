...
ScaledComfortNoiseGenerator::ScaledComfortNoiseGenerator(Aec3Optimization optimization,
...
ScaledComfortNoiseGenerator::~ScaledComfortNoiseGenerator() = default;

void ScaledComfortNoiseGenerator::Compute(
  ...
  if (!saturated_capture) {
    ...
    for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
      for (auto& n : N2_[ch]) {
        n = std::min(kNoiseCeiling, std::max(n, kNoiseFloor));
      }
      if (N2_initial_) {
